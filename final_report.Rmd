...
We also were looking to explore questions related to hours of operation on each day of the week, however we decided not to move forward with this analysis because there was a lot of missing data in the variables.
...
Because we were interested in the binary outcome of whether a condom was available or not, we decided to analyze the data using two logistic regression models. One model predicted the availability of male condoms and the other predicted the availability of female condoms. We included `borough`, `partnertype`, and `partner_subtype` in the model to see if these variables were significant in predicting the availability of a condom. Our models and conclusions are available on [Statistical Analysis](lm.html)
...
Coding errors can sometime feel like looking for a needle in a haystack. Throughout our project we did encounter some minor issues with code, inability to knit, and lack of knowledge in some areas of R that we incorporated in our analysis. Particularly, the intricacies of R Shiny. 
...
Although the data was pretty clean, it would’ve been nice if there were more continuous variables, especially for our regression models. This would've allowed for greater variety in the types of analysis and visualizations we could perform.
